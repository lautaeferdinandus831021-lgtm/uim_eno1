...
def test_order_entity():
    from domain.trading.entities.order import Order, OrderSide, OrderStatus
    o = Order(side=OrderSide.BUY, symbol="BTCUSDT", size=0.001)
    assert o.is_buy
    assert o.status == OrderStatus.PENDING
    o.mark_filled("order123", 0.05)
    assert o.status == OrderStatus.FILLED
    assert o.order_id == "order123"

def test_trade_entity():
    from domain.trading.entities.trade import Trade
    t = Trade(user_id=1, mode="spot", side="buy", symbol="BTCUSDT", entry_price=68000, size=0.001)
    assert t.status == "open"
    t.close(69000, fee=0.01)
...
def test_position_entity():
    ...
    p = Position(symbol="BTCUSDT", side="long", size=0.001, entry_price=68000, current_price=69000)
    assert p.pnl > 0
    assert p.pnl_pct > 0
    p.update_price(67000)
    assert p.pnl < 0
...
def test_settings_defaults():
    ...
    assert s.MAX_REQUESTS_PER_MINUTE == 60
    assert s.MAX_LOGIN_ATTEMPTS == 5
    assert s.SESSION_LIFETIME == 86400
...
def test_generate_token_unique():
    ...
    assert len(h) == 64
    assert hash_token("test123") == h
    assert hash_token("test124") != h
