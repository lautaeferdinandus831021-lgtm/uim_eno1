...
interface OverlayPoint { time: number; value: number; color?: string; }
interface Overlays {
...
export function CandleChart({ candles, overlays, signal, price, height = 400 }: Props) {
  ...
  useEffect(() => {
    ...
    const chart = createChart(containerRef.current, {
      ...
      crosshair: { mode: 0 },
      rightPriceScale: { borderColor: "#1a2536", scaleMargins: { top: 0.1, bottom: 0.1 } },
  ...
  useEffect(() => {
    ...
    if (overlays?.ema_fast?.length && chartRef.current) {
      ...
      s.setData(overlays.ema_fast.map((p) => ({ time: p.time as Time, value: p.value })));
    }
    if (overlays?.ema_slow?.length && chartRef.current) {
      ...
      s.setData(overlays.ema_slow.map((p) => ({ time: p.time as Time, value: p.value })));
    }
